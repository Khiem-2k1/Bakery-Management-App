    ...
    public partial class changePasswordView : UserControl
    {
        ...
        private void btnDoiMatKhau_Click(object sender, EventArgs e) //khi ta nhan vao nut doi mk se goi toi event nay
        {
            if(IsValid()) //N?u hàm ki?m tra tr? v? true
            {
                ...
                DataSet ds = db.changePassword(ref err, this.user.username, txtOldPassword.Text, txtNewPassword.Text, txtConfirmPassword.Text); //t?o bi?n ds ki?u DataSet = cách g?i t?i hàm changePassword t? UserDAO
                string status = ds.Tables[0].Rows[0]["status"].ToString();
                MessageBox.Show(status, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);//Hi?n h?p thông báo v?i tiêu ?? thông báo và nút OK
