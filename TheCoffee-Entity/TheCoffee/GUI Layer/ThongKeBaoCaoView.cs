...
namespace TheCoffee
{
    public partial class ThongKeBaoCaoView : UserControl
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (check == true)
            {
                dgvDSTransaction.DataSource = statisticDAO.getAllBaoCaoDoanhThu();
            }
            else
            {
                dgvDSTransaction.DataSource = statisticDAO.getBaoCaoDoanhThuDate(dateFrom.Text, dateTo.Text);
            }
        ...
        private void dgvDSTransaction_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            dgvOrderDetail.DataSource = orderDAO.GetOrderDetail(username, transactionID);//L?y thong tin c?a ??n hàng dùng 2 giá tr? v?a ki?m ???c và truy?n vao datasource c?a dgvOrderDetail
        }
    }
}
