...
namespace TheCoffee
{
    public partial class MainFormCustomer : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            timKiemSanPhamTheoTenView frm =  new timKiemSanPhamTheoTenView(txtSearch.Text, user);
        ...
        private void LoadGioHang(DBProvider.Order order)
        {

            if (user.orderTable.Rows.Count == 0)
            {
                user.orderTable.Rows.Add(order.productID, order.pro_name, order.or_quantity, order.or_price);
            }
            else
            {
                int vitri = -1;
                ...
                if (vitri != -1)
                {
                    ...
                    user.orderTable.Rows[vitri]["SoLuong"] = (currQuantity + order.or_quantity);
        ...
        {
            lblNamePage.Text = "Quán lí đơn hàng";
            openChildForm(new QuanliDonHangCaNhan(user));
        }
