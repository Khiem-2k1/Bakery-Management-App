...
namespace TheCoffee
{
    public partial class timKiemSanPhamTheoTenView : UserControl
    {
        ...
        public timKiemSanPhamTheoTenView(string productname, User user)
        {
            InitializeComponent();
        ...
        {
            dgvTimKiemSanPham.DataSource = db.timKiemSanPhamTheoTen(this.productName.Trim());
        }
        ...
        private void dgvTimKiemSanPham_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Product product = db.getProduct(Convert.ToInt32(dgvTimKiemSanPham.Rows[r].Cells[0].Value));

            lblProductID.Text = product.id.ToString();
            lblProductName.Text = product.pro_name;
            lblProductPrice.Text = product.pro_price.ToString();
            lblTotalPrice.Text = (product.pro_price * (float)numericUpDown1.Value) + "VND";
            numericUpDown1.Maximum = product.pro_quantity;
            numericUpDown1.Value = 1;
            
            lblMax.Text = "(Còn lại: " + product.pro_quantity + ")";
        ...
        private void btnDatHang_Click(object sender, EventArgs e)
        {
            ...
            order.or_quantity = quantity;
            order.pro_productID = productID;
            order.us_username = user.us_username;
            order.pro_name = lblProductName.Text;
            order.or_price = float.Parse(lblProductPrice.Text) * (float)numericUpDown1.Value;
